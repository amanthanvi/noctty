...
function Open-ThemeQuery([IntPtr]$Host, [string]$Query, [DateTime]$Deadline, $Process) {
    Invoke-StatefulCommand $Host 1901
    $script:PaletteThemeHost = $Host
    ...
    $edit = Get-StatefulChildren $Host | Where-Object Id -eq 2002 | Select-Object -First 1
...
try {
    ...
    $host = Wait-StatefulHost $run $deadline
    $surface = Get-StatefulSurface $host; if ($null -eq $surface) { throw 'No terminal surface HWND.' }
    $originalPixel = Get-StatefulPixel $surface.Hwnd
    $edit = Open-ThemeQuery $host '0x96f' $deadline $run.Process
    ...
    $edit = Open-ThemeQuery $host '0x96f' $deadline $run.Process
    Invoke-StatefulCommand $host 2003
    ...
    Close-StatefulHost $host $run $deadline
