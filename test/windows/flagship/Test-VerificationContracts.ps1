...
foreach ($contract in @(
    @{ Pattern = '(?s)\$firstPortableManifestVersion = \[version\]''1\.3\.124''.*?\$requiresPortableManifests = \(\$releaseVersion -ge \$firstPortableManifestVersion\)'; Description = 'published verifier preserves the first portable-manifest release boundary' },
    @{ Pattern = '(?s)if \(\$requiresPortableManifests\).*?-Kind manifest.*?\$expectedAssetCount = if \(\$requiresPortableManifests\) \{ 10 \} else \{ 8 \}.*?\$expectedNames\.Count -ne \$expectedAssetCount'; Description = 'published verifier requires eight legacy assets or ten manifest-era assets' },
    ...
    @{ Pattern = '(?s)\$signatureEvidence\.Add\(\(Assert-PublishedSignature.*?Setup \$architecture.*?if \(\$requiresPortableManifests\).*?Portable manifest \$architecture.*?if \(\$requiresPortableManifests\).*?Assert-PortableManifestMatchesPayload.*?foreach \(\$relativePath.*?\$signatureEvidence\.Add\(\(Assert-PublishedSignature.*?\$expectedSignatureCount = if \(\$requiresPortableManifests\) \{ 10 \} else \{ 8 \}.*?\$signatureEvidence\.Count -ne \$expectedSignatureCount'; Description = 'published verifier validates eight legacy signatures or ten manifest-era signatures' },
