...
fork's older `PageList` API and was dropped. **Seven picks are certified**: they
apply cleanly, build, and leave the full test suite at its known baseline
(3770/3841 passing, the one pre-existing `Command: custom env vars` failure).
...
### Certified slice — applied in the 2026-08 window
...
- No merge commit, no rebase of fork history, and no write to the `upstream`
  remote. The only source changes are the seven certified cherry-picks listed
  above, each as its own commit.
...
- The seven certified picks were validated by build and the full test suite,
  not by interactive runtime observation. The `renderer: fix preedit range
width` pick changes IME preedit cell suppression by one cell in the shared
  generic renderer; it is covered by unit tests but was not confirmed against a
  live IME session.
