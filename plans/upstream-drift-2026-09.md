...
In every case the production hunk is unmodified. On the test side, only setup
and the one `PageList` integrity-assert expression were adapted to the fork's
older Zig and `PageList` API; no assertion's expected value changed.
...
| Row-storage recycling and page pool        | `b31fbc846474`, `e01e75bbb228`, `028af92ce387`                                                                                 | All conflict in `PageList.zig`; `b31fbc846474` also needs `src/terminal/snapshot/testdata/complete-v1.hex`, which the fork does not carry, and `e01e75bbb228` adds `datastruct/untouched_pool.zig`. |
