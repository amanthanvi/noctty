...
/// The Windows font discovery implementation lives in discovery_windows.zig.
/// Windows builds route `font.discovery` directly at that file (see main.zig);
/// this re-export keeps the `Discover` switch above pointing at the single
/// source of truth.
pub const Windows = @import("discovery_windows.zig").Windows;
...
test {
    // Include the Windows discovery tests. Its pure helper tests run on
    // all hosts; the end-to-end tests skip themselves off-Windows.
    _ = @import("discovery_windows.zig");
}
