        ...
        function ssh --wraps=ssh --description "SSH wrapper with Ghostty integration"
            ...
            if contains ssh-terminfo $features
                ...
                if test -n "$ssh_hostname"
                    ...
                    if test -x "$GHOSTTY_BIN_DIR/noctty"; and "$GHOSTTY_BIN_DIR/noctty" +ssh-cache --host="$ssh_target" >/dev/null 2>&1
                        ...
                        if test -n "$ssh_terminfo"
                            ...
                            ' 2>/dev/null
                                ...
                                if test -x "$GHOSTTY_BIN_DIR/noctty"
                                    "$GHOSTTY_BIN_DIR/noctty" +ssh-cache --add="$ssh_target" >/dev/null 2>&1; or true
                                end
                            else
