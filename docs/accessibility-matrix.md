...
| Focus-region cycle | The landed region announces itself as a focus change | Asserted, **not yet executed** — F6, Shift+F6 and Escape move real Win32 focus between the terminal and the tab strip, and the landed TabItem reports IsKeyboardFocusable and HasKeyboardFocus | not yet measured | not yet measured | not yet measured | New; the harness run recorded above predates these assertions. Each landing HWND raises its own focus-changed event, so the announcement is whatever that element already announces — see its own row |
...
edit), the custom-painted caption buttons, the profile picker and
tab-overview rows, and context menus. Overlays such as the command
